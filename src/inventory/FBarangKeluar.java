 ...
 * @author indak
 */
public class FBarangKeluar extends javax.swing.JFrame {
    ...
    private void tblBarangKeluarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblBarangKeluarMouseClicked
        ...
        if (selectedRow != -1) {
            try {
                String transactionCode = tblBarangKeluar.getValueAt(selectedRow, 0).toString();
                String productName = tblBarangKeluar.getValueAt(selectedRow, 1).toString();
                String supplierName = tblBarangKeluar.getValueAt(selectedRow, 2).toString();
                String amountStr = tblBarangKeluar.getValueAt(selectedRow, 3).toString();
                String dateString = tblBarangKeluar.getValueAt(selectedRow, 4).toString();
    ...
    private void btnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSimpanActionPerformed
        ...
        if (code.isEmpty() || selectedProductString.isEmpty() || selectedSupplierString.isEmpty() || selectedProductString == "- Pilih Barang -" || selectedSupplierString == "- Pilih Supplier -" || selectedDate == null) {
            ...
            try {
                ...
                // Get the current stock of items from the products table
                String getCurrentStockQuery = "SELECT stock FROM products WHERE code = ?";
                PreparedStatement getCurrentStockStmt = c.prepareStatement(getCurrentStockQuery);
                getCurrentStockStmt.setString(1, selectedProductCode);
                ResultSet currentStockResult = getCurrentStockStmt.executeQuery();

                int currentStock = 0;
                if (currentStockResult.next()) {
                    currentStock = currentStockResult.getInt("stock");
                }

                // Check whether there is enough stock for outgoing transactions
                ...
                if (newStock >= 0) {
                    ...
                    if (rs.next() && rs.getInt(1) > 0) {
                        ...
                        insertStmt.setString(6, "Out");
                        insertStmt.executeUpdate();

                        JOptionPane.showMessageDialog(this, "Data transaksi berhasil ditambahkan");
                    }
                } else {
                    JOptionPane.showMessageDialog(this, "Stok tidak mencukupi untuk transaksi ini");
                }
    ...
    public void loadDatabase() {
        ...
        try {
            ...
            String sql = "SELECT t.code AS transaction_code, p.name AS product_name, s.name AS supplier_name, t.amount, t.date "
                    ...
                    + "LEFT JOIN products p ON t.product_id = p.id "
                    + "LEFT JOIN suppliers s ON t.supplier_id = s.id "
                    + "WHERE t.type = 'Out'"; // Hanya transaksi barang keluar ('Out')
            ResultSet r = s.executeQuery(sql);
        ...
        try {
            ...
            // Output the results of the query into a combobox
            while (r.next()) {
                String code = r.getString("code");
                String name = r.getString("name");
                cbBarang.addItem(code + " - " + name); // Tambahkan ke combo box
            }
        ...
        try {
            ...
            while (r.next()) {
                String code = r.getString("code");
                String name = r.getString("name");
                cbSupplier.addItem(code + " - " + name); // Tambahkan ke combo box
            }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FBarangKeluar.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
